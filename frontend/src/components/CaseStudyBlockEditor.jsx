    ...
    <div className="border rounded p-3 mb-4">
      ...
      {block.type === "image" && (
        <div className="flex flex-col gap-2">
          ...
          />
          {block?._file ? (
            <img src={URL.createObjectURL(block._file)} alt={block.alt || ''} className="object-contain w-full max-h-40 rounded border" style={{ marginTop: 8 }} />
          ) : (block?.src || block?.url) ? (
            <img src={block.src || block.url} alt={block.alt || ''} className="object-contain w-full max-h-40 rounded border" style={{ marginTop: 8 }} />
          ) : null}
